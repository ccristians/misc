...
__NTH (atoi (const char *__nptr))
{
  ...
  4007a5:	75 1e                	jne    4007c5 <main+0xa5>
#else
            if (buffer[j]) {
#endif
                result += func1(j);
  ...
  4007ae:	41 01 c5             	add    %eax,%r13d
    ...
    for (i = 0; i < RUN_CNT; i++) {
        for (j = 0; j < size; j++) {
  4007b1:	48 ff c3             	inc    %rbx
  4007b4:	41 39 df             	cmp    %ebx,%r15d
  4007b7:	75 e7                	jne    4007a0 <main+0x80>
        exit(EXIT_FAILURE);
    }
    ...
    for (i = 0; i < RUN_CNT; i++) {
  4007b9:	ff c5                	inc    %ebp
  4007bb:	81 fd 10 27 00 00    	cmp    $0x2710,%ebp
  4007c1:	75 cd                	jne    400790 <main+0x70>
  4007c3:	eb 32                	jmp    4007f7 <main+0xd7>
#ifdef EXPECTED_VALUE
            if (__builtin_expect(buffer[j], EXPECTED_VALUE)) {
#else
            if (buffer[j]) {
#endif
                result += func1(j);
  4007c5:	89 df                	mov    %ebx,%edi
  4007c7:	e8 a4 00 00 00       	callq  400870 <func1>
  4007cc:	eb e0                	jmp    4007ae <main+0x8e>
